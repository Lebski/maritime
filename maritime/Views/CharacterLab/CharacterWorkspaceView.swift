...
struct CharacterSetupPlaceholderView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("Ready to generate portraits")
                .font(.system(size: 17, weight: .bold))
                .foregroundStyle(Theme.textPrimary)
            Text("We'll send your description to recraft-v4 and generate \(character.portraitCount) variations.")
                .font(.system(size: 12))
                .foregroundStyle(Theme.textSecondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 380)
            Button(action: { vm.startGeneration(characterID: character.id) }) {
